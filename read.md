...
│   ├── edge_results/              # Sobel/Laplacian/Canny/Prewitt outputs for subset_1 (gitignored, regenerated by script)
...
`images/blurred`, `images/subsets`, and `images/edge_results` are
listed in `.gitignore` because together they hold thousands of files
generated from the committed source images. Run `python src/main.py`
from the project root to regenerate them (and everything else in the
...
1. `equalized_scale06_comparison.png` — bright, high-contrast, **unblurred**.
   Canny produces the cleanest, thinnest edges of all four detectors here.
2. `equalized_rgb_sigma_3.0_comparison.png` — the same underlying image,
   **heavily blurred (sigma = 3.0)**. Canny's output is now almost entirely
   blank, while Sobel and Prewitt still show a faint outline. This pair is
   the clearest illustration of blur causing Canny's failure.
3. `grayscale_comparison.png` — dark, low-contrast, **unblurred**. Canny
   is already close to blank here, showing that low intrinsic contrast
   alone (without any blur) is enough to break it.
4. `hsv_comparison.png` — a high local-contrast, unblurred image (raw HSV
   channels displayed as if BGR). Canny picks up real structure but also
   noise, since noise gradients can also clear a low-enough threshold.
5. `hsv_rotate186_sigma_0.5_comparison.png` — lightly blurred (sigma = 0.5).
   Canny is visibly weaker than in the unblurred case but not yet blank.
6. `hsv_rotate186_sigma_3.0_comparison.png` — heavily blurred version of
   the same rotated image. Canny is essentially blank.
